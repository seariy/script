...
EOF
    # Inject right before the exit of udhcpc.sh
    sed -i '/exit 0/e cat /tmp/udhcpc_hook.txt' "$UDHCPC_SCRIPT"
...
if [ -f "$STORAGE_DEF" ]; then
  echo ">> Injecting ap_watchdog.sh into storage_default.sh..."
  cat << 'EOF' >> "$STORAGE_DEF"
